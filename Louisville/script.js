(function() {
	...
	// 2. UNIFIED LIGHTBOX SYSTEM (FOR ALL POP-OUT REVIEWS)
	window.openPortalLightbox = function(title, label, meta, story, imageSrc, externalUrl) {
		...
		if(imageSrc) {
			...
			mediaBox.classList.replace('hidden', 'flex');
		} else {
			mediaBox.classList.replace('flex', 'hidden');
		}
		...
		mask.querySelector('div').classList.remove('scale-95');
	...
	window.closePortalLightbox = function() {
		...
		mask.querySelector('div').classList.add('scale-95');
		...
		try {
			...
			if(Array.isArray(rawEvents) && rawEvents.length > 0) {
				...
				if(structuredEvents.length === 0) {
					listNode.innerHTML = `<div class="text-center text-xs py-6 text-neutral-400 italic">No scheduled entries active.</div>`;
					return;
				}
				...
				if(list.length > 0 && target) {
					...
					list.forEach(h => {
						...
						row.onclick = () => {
							...
							document.getElementById('lightbox-media-box').classList.add('hidden');
